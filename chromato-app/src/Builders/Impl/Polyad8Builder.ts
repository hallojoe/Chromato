...
export class Polyad8Builder implements IColor2DBuilder {
  public combine(color2D: IColor2D, size: ISize = { width: 4, height: 4}): IGeneratedColorSet {
    ...
    const generatedColorSet = new GeneratedColorSet("polyad eight");
    const backgrounds = backgroundColor.polyad(size.width + 1).map(v => v.toRgbString()).splice(1);
    const foregrounds = foregroundColor.polyad(size.height + 1).map(v => v.toRgbString()).splice(1);    
    for(let i = 0; i < size.width; i++) {
      ...
      for(let ii = 0; ii < size.height; ii++) {
        const generatedColor2D = new Color2D(
          backgrounds[i], 
          foregrounds[ii]); 
        generatedColor2D.description = `polyad eight`;
